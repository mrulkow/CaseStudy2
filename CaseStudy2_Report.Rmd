...
2. Click the **Knit** button. (If you receive an error, check out the section titled "Knitting Error" found below.)
...
If you attempt to knit the document and receive the error message below, this means that you have an unrecognized character within your document. The error message will typically tell you which character is unrecognized (in the example below, a "mu" symbol is used somewhere within the document). This unrecognized character could be a "pi" symbol, a "mu" symbol, a hyphen or dash, a set of quotation marks, a not equal sign, etc. Locate the unrecognized character and replace it (or delete it). Your document should knit when all unrecognized characters are replaced (or deleted). Reach out to the instructional team in lab, in office hours, or via Piazza for additional help.
